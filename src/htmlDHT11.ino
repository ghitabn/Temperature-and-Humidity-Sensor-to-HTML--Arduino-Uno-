...
Description:  periodic readings from a DHT11 temperature and humidity sensor 
              ...
              Sensor readings (temperature and humidity) as well as computed 
              values (heat indexes) could be accessed via a minimalist webpage 
              hosted on a SD card. The webpage refreshes sensor data using 
              AJAX and XML.
...
void XML_response(EthernetClient cl)
{
  ...
  {
    // sensor-specific computations
    float heatIndexC = dht.computeHeatIndex(tempC, humidity, false); // heat index (Celsius)
    float heatIndexF = dht.computeHeatIndex(tempF, humidity); // heat index (Fahrenheit)
